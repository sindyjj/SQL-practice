...
트리거 자체를 트랜젝션의 일부로 처리하기 때문에 COMMIT이나 ROLLBACK을 포함 할 수 없습니다.
*/
...
CREATE TABLE tbl_user_backup(
    ...
    m_type VARCHAR2(10), -- 변경 타입(MODIFY타입)
    m_user VARCHAR2(20) -- 변경할 사용자
);
...
DECLARE
    v_type VARCHAR2(10);
BEGIN
    -- 현재 트리거가 발동된 상황이 UPDATE인지를 파악하는 조건문
    ...
    -- 본격적인 실행 구문 작성 (backup 테이블에 값을 INSEET ->
    -- 원본 테이블에서 UPDATE OR DELETE 된 사용자 정보 및 기타 정보)
    INSERT INTO tbl_user_backup
    VALUES(:OLD.id, :OLD.name, :OLD.address, sysdate, v_type, USER()); -- USER(): 현재 유저 정보 가져오기
END;

-- 확인 : 
INSERT INTO tbl_user VALUES('test01','won','부산');
INSERT INTO tbl_user VALUES('test02','you','서울');
INSERT INTO tbl_user VALUES('test03','sung','외계');
...
INSERT INTO tbl_user VALUES('test06','메롱이','대전');
INSERT INTO tbl_user VALUES('test07','초롱이','경주');
...
INSERT INTO product VALUES(product_seq.NEXTVAL, '훈모' , 1 ,100000);
INSERT INTO product VALUES(product_seq.NEXTVAL, '삼경' , 100,100);
INSERT INTO product VALUES(product_seq.NEXTVAL, '한솔' , 10 ,10000);
...
BEGIN
    ...
    EXCEPTION 
        WHEN quantity_shortage_exception THEN
            ...
            -- 두번째 매개값: 에러 메세지
            RAISE_APPLICATION_ERROR(-20001,'주문하신 수량보다 제고가 적어서 주문할 수 없습니다.');
        WHEN zero_total_exception THEN
            RAISE_APPLICATION_ERROR(-20002,'주문하신 상품의 제고가 없어 주문할수 없습니다.');
END;
