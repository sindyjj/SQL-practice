...
FROM info
JOIN auth -- INNER 생략시 그냥 JOIN을 쓴다면 default 값이 INNER
...
--  이문장에서 OUTER를 생략해도 OUTER 가 적용된다 하지만 LEFT와 JOIN은 써줘야한다.
ON i.auth_id = a.auth_id;
...
--좌측 테이블 과 우측 테이블 데이터를 모두 읽어 중복된 데이터는 삭제되는 외부 조인
SELECT
    *
...
ORDER BY e1.employee_id ASC;               -- 이해 안감 복습 필수....
